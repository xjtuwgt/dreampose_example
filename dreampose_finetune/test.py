...
from dual_encoder_pipeline import StableDiffusionImg2ImgPipeline
import argparse
from torchvision import transforms
import torch
import cv2, PIL, glob, random
import numpy as np
# from  torch.cuda.amp import autocastsampler
...
from model import get_unet, Embedding_Adapter

parser = argparse.ArgumentParser()
parser.add_argument("--folder", default='./demo/custom-chkpts', help="Path to custom pretrained checkpoints folder.", )
parser.add_argument("--pose_folder", default='../UBC_Fashion_Dataset/valid/91iZ9x8NI0S.mp4',
...
parser.add_argument("--epoch", type=int, default=44, required=True,
                    help="Pretrained custom model checkpoint epoch number.", )
parser.add_argument("--key_frame_path", default='../UBC_Fashion_Dataset/dreampose/91iZ9x8NI0S.mp4/key_frame.png',
...
parser.add_argument("--strength", type=float, default=1.0, required=False,
...
save_folder = os.path.join('./demo/result')
if not os.path.exists(save_folder):
    os.mkdir(save_folder)
...
frame_numbers = sorted([int(path.split('frame_')[-1].replace('_densepose.npy', '')) for path in
                        glob.glob(f'{args.pose_folder}/frame_*.npy')])
